...
/**
 * PHP version 5
 * This script has been modified by Chris Noden
 *
 * @package    UASparser
 ...
 *
 * @todo    Improve the download process and exception handling
 * @todo    Add a disable function to the project xml
 */
...
class UASparser
{
    private $updateInterval   = 86400; // 1 day
    ...
    public function __construct($cacheDirectory = null, $updateInterval = null) {
        $this->updateInterval = intval(86400*7); // 7 days
        if ($cacheDirectory) $this->SetCacheDir($cacheDirectory);
        if ($updateInterval) $this->updateInterval = $updateInterval;
    }

    /**
     * Turn off fetching the latest data from the net
     * Just use the local version that came with the SAL framework
     */
    public function skipInternet()
    {
        $this->_skipInternet = true;
    }
    ...
    private function _loadData() {
        if ($this->_skipInternet == false && !isset($this->_cache_dir)) {
            $this->_skipInternet = true;
        }

        // if we tried to download the file and failed recently then don't try again
        if (file_exists($this->_cache_dir.DIRECTORY_SEPARATOR.'failed') && (date('U') - filemtime($this->_cache_dir.DIRECTORY_SEPARATOR.'failed')) < 300) {
            $this->_skipInternet = true;
        }

        // Are we to use the framework supplied data (skipInternet == true)
        if ($this->_skipInternet == true) {
            return @parse_ini_file(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'uasdata.ini', true);
        }
        ...
        if (file_exists($this->_cache_dir.'/uasdata.ini')) {
            ...
            throw new \NDN\SAL\SalException('ERROR: No datafile (uasdata.ini in Cache Dir), maybe update the file manually');
        }
    }
    ...
    private function _downloadData() {
        ...
        if (is_string($ini) && strlen($ini) > 100) {
            ...
            if (file_exists($this->_cache_dir.DIRECTORY_SEPARATOR.'failed')) unlink($this->_cache_dir.DIRECTORY_SEPARATOR.'failed');
        } else {
            touch($this->_cache_dir.DIRECTORY_SEPARATOR.'failed');
            throw new \NDN\SAL\SalException("Failed to download UAS data from ". self::$_ini_url);
        }
    }
    ...
    private function get_contents($url, $timeout = 5)
    {
        ...
        if(ini_get('allow_url_fopen')) {
            try {
                ...
                $ini = @file_get_contents($url, false, $ctx);
            } catch (\Exception $ex) {
                return false;
            }
            return $ini;
